...
def get_mon_conf_alone(fname):
    ...
    d_config['mail_server']             = config.get("ALONE", "mail_server")
    d_config['mail_port']               = config.get("ALONE", "mail_port")
...
def get_mon_conf_cluster(fname):
    ...
    d_config['mail_server'] = config.get("CLUSTER", "mail_server")
    d_config['mail_port']   = config.get("CLUSTER", "mail_port")
...
def monitor_alone(fname,debug):
    ...
    while True:
       ...
       if get_seconds(start_time_db_info) >= 60:
...
def monitor_cluster(fname,debug):
    i_counter = 0
    #read configure
    ...
    start_time_tab_var   = datetime.datetime.now()
    start_time_db_health = datetime.datetime.now()
    start_time_db_health_slave = datetime.datetime.now()
    start_time_slow_sql_hz = datetime.datetime.now()
    while True:
        #sleep 3 seconds
        time.sleep(3)

        #check master and slave relay
        ...
        #gather slow sql for Once every two minutes
        if get_seconds(start_time_meta_lock) >= 15:
           gather_metadata_lock_info(config)
           start_time_meta_lock = datetime.datetime.now()
        ...
        if get_mon_period(config):
            ...
            #every day 12 hours send table variable rate,43200
            if get_seconds(start_time_tab_var) >= 14400:
                v_content = send_tab_var_rate(config)
                if v_content != '':
                    send_mail(config['send_user'], config['send_pass'], config['acpt_user'], config['cc_user'],
            ...
            #Master:every half hour or go wrong at work send db health report
            n_status = 0
            v_db_health_title=''
            ...
            #Slave:every half hour or go wrong at work send db health report
            n_status_slave = 0
            v_health_title_slave = ''
            if get_seconds(start_time_db_health_slave) >= 180:
                start_time_db_health_slave = datetime.datetime.now()
                if if_exceed_threshold_slave(config):
...
def check_sync_relay(config):
    ...
    #主库更新时钟表，然后马上查询主从库数据是否一致
    cr_master.execute('update devops.heart set x=now()')
    config['db1'].commit()
    cr_master.execute("SELECT x,UNIX_TIMESTAMP(x) FROM devops.heart limit 1")
    ...
    cr_slave.execute("SELECT x,UNIX_TIMESTAMP(x) FROM devops.heart limit 1")
...
def send_tab_var_rate(config):
    ...
    cr = config['db1'].cursor()
    if config['mon_db'] == '*':
        p_sql = """select b.schema_name as "库名"，
                          a.TABLE_NAME  as "表名",
                          a.table_rows  as "表行数",
                    ...
                    FROM (SELECT * FROM t_tab_rate 
                           WHERE tjrq=(SELECT MIN(tjrq) FROM t_tab_rate 
                                       WHERE DATE_FORMAT(tjrq, '%Y%m%d') =DATE_FORMAT(date_sub(curdate(),interval 1 day), '%Y%m%d')) ) a,
                         (SELECT * FROM t_tab_rate 
                           WHERE tjrq=(SELECT MAX(tjrq) FROM t_tab_rate 
...
def get_metadata_lock(config):
    ...
    nowTime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cr = config['db1'].cursor()
    sql="""select count(0) from t_metadata_lock where send_mail='N'"""
    cr.execute(sql)
    rs=cr.fetchone()
    if rs[0]==0:
      return ''
    ...
    for i in rs:
      ...
      tbody=tbody+row
    v_html =get_html_contents(config,thead,tbody)
    v_upd_sql="update t_metadata_lock set send_mail='Y' where id in({0})".format(ids[0:-1])
...
def write_slow_sql_hz_xls(config):
    ...
    p_sql="""SELECT   concat((@rowNum:=@rowNum+1),'') as "序号",x.*
             FROM(SELECT
                      ...
                      STATEMENT  AS "优化建议",
                      STATEMENT  AS "执行语句"
                FROM t_slow_sql,(select (@rowNum:=0)) b
...
def get_exec_times(config,p_id):
    ...
    p_sql="""SELECT id,esalpse_time
              ...
              where created_date BETWEEN CONCAT(DATE_FORMAT(now(),'%Y-%m-%d'),' 0:0:0') AND CONCAT(DATE_FORMAT(now(),'%Y-%m-%d'),' 23:59:59')
                  AND statement=(select statement from t_slow_sql where id={0})
...
def gather_slow_sql(config):
    ...
    if config['mon_db'] == '*':
       v_sql="""insert into t_slow_sql(STATEMENT,esalpse_time,created_date,db,host)
...
def gather_slow_sql_slave(config):
    ...
    for i in range(len(rs2)):
       v_ins_sql="""insert into t_slow_sql(STATEMENT,esalpse_time,created_date,db,host)
                     values('{0}','{1}','{2}','{3}','{4}')
...
def gather_db_info(config):
    ...
    v_sql="""insert into t_db_monitor(connection_total,connection_active,tjrq,host) 
...
def gather_tab_var_info(config):
    nowTime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cr = config['db1'].cursor()
    v_sql="""insert into t_tab_rate(schema_name,table_name,table_rows,table_size,tjrq)
               select   table_schema,
                        table_name,
                        table_rows,
...
def set_block_tables(config):
    nowTime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cr = config['db1'].cursor()
    cr.execute("truncate table t_metadata_lock_table")
    sql="""insert into t_metadata_lock_table(db,tname)
...
def gather_metadata_lock_info(config):
    ...
    if rs[0]>0:
       ...
       sql="""INSERT INTO t_metadata_lock(id,user,host,db,command,time,state,info,status,lock_type,send_mail,start_time,gather_time)
              ...
              SELECT id,user,host,db,command,time,state,info,'blocking',
               ...
               FROM information_schema.processlist p
                WHERE id IN(SELECT trx_mysql_thread_id FROM information_schema.innodb_trx)
                   AND exists(select 1 from t_metadata_lock_table m 
                              where m.db=p.db and instr(p.info,concat(' ',m.tname))>0)
           """.format(nowTime,nowTime)
       cr.execute("truncate table t_metadata_lock_table")
       cr.execute(sql)
       cr.execute("select count(distinct status) from t_metadata_lock where gather_time='{0}'".format(nowTime))
       rs2=cr.fetchone()
       if rs2[0]!=2:
         cr.execute("delete from t_metadata_lock where gather_time='{0}'".format(nowTime))
...
def if_exceed_threshold_slave(config):
    if (get_active_conns_slave(config)  >= int(config['active_connections'])) or \
       (get_slow_records_slave(config)  >= int(config['slow_query_numbers'])) or \
       (get_txn_records_slave(config)   >= int(config['max_trans_numbers']))    or \
       (get_block_records_slave(config) >= int(config['lock_enqueue_numbers'])) or \
       (get_total_conns_slave(config)   >= int(config['total_connections'])):
       return True
    else:
       return False
...
def get_db_health(config):
    ...
    v_tbody ='''<tr><td width=15%><b>环境描述</b></td> <td width=40%>{0}</td></tr>
	        <tr><td width=15%><b>数据库</b></td> <td width=40%>{1}</td></tr>
   	        <tr><td width=15%><b>用户名</b></td> <td width=40%>{2}</td></tr>
                <tr><td width=15%><b>连接数总数</b></td> <td width=40%>{3}</td></tr>
                <tr><td width=15%><b>活动连接数</b></td> <td width=40%>{4}</td></tr>
                <tr><td width=15%><b>慢查询数量</b></td> <td width=40%>{5}</td></tr>
                <tr><td width=15%><b>事务记录数</b></td> <td width=40%>{6}</td></tr>
                <tr><td width=15%><b>锁等待数量</b></td> <td width=40%>{7}</td></tr>
                <tr><td width=15%><b>预警阀值数</b></td> <td width=40%>{8}</td></tr>
    ...
    v_html='''<html>
                  <head>
                       <style type="text/css">
                                    p {width:60%;}
                                    body {border :1px;}
                       .xwtable {width: 60%;border-collapse: collapse;border: 1px solid #ccc;padding-left:50px;}
                       .xwtable thead td {font-size: 12px;color: #333333;
                                      text-align: center;background: url(table_top.jpg) repeat-x top center;
                                  border: 1px solid #ccc; font-weight:bold;}
                       .xwtable thead th {font-size: 12px;color: #333333;
...
def gather_indexes(fname):
    ...
    for k in range(len(tabs)):
       ...
       #v_sql_del="delete from syncer.t_db_indexes where schema_name='{0}' and table_name='{1}'".format(config['index_schema'],tabs[k])
       #cr.execute(v_sql_del)
       cr.execute(v_sql_index)
       ind_rs=cr.fetchall()
       for j in ind_rs:
          v_sql_ins="""insert into t_db_indexes(schema_name,table_name,non_unique,key_name,seq_in_index,column_name,is_null,index_type,created)
    ...
    for f in config['binlog_file'].split(','):
        ...
        for i in rs:
           if i[2]=='Query' and ('create' in i[5].lower() or 'alter' in i[5].lower() \
              ...
              v_sql_ins="""insert into t_db_binlog(log_name,begin_pos,event_type,server_id,end_pos,statement,db) values('{0}','{1}','{2}','{3}','{4}','{5}','{6}')
        ...
        print("\rTotal rec:{0},Process rec:{1}({2}),Complete:{3}%".format(len(rs),i_counter2,i_counter,round(i_counter2/len(rs)*100,2)),end='')
        config['db'].commit()
        print("\nbinlog '{0}' parse to table:'{1}' success!".format(f,'t_db_binlog'))
    cr.close()
...
def main():
    ...
    #process
    if mode == "alone":
       monitor_alone(config,debug)
    elif mode == "cluster":
       monitor_cluster(config,debug)
