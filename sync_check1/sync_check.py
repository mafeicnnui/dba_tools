...
def get_html_contents(config):
    ...
    v_sql  = '''SELECT 
                  ...
                  CASE WHEN ABS((t.bi_flow-t.big_flow))/t.bi_flow <=0.05 THEN '√' ELSE  '×' END AS '客流比对',
                  CASE WHEN ABS((t.bi_park-t.big_park))/t.bi_park <=0.05 THEN '√' ELSE  '×' END AS '车流对比', 
...
def get_bi_park_stats(config,p_market_id):
    ...
    if p_market_id=='218':
        v_stats = '''select COUNT( platenumpic) 
                      ...
                      where controlid IN (2,5,9,11,14,20,21,23,24,25,26,28,29)
                        and platenumpic != ''
                        and platenumpic is not null 
...
def check_valid(config):
    ...
    sql='''SELECT COUNT(0)
            FROM (SELECT 
                      market_id,
                      CASE WHEN ABS((t.bi_flow-t.big_flow))/t.bi_flow <=0.05 THEN '√' ELSE  '×' END AS 'flow',
                      CASE WHEN ABS((t.bi_park-t.big_park))/t.bi_park <=0.05 THEN '√' ELSE  '×' END AS 'park' 
                    FROM  sync.t_sync_stats t
                    WHERE tjrq >= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 2 DAY),'%Y-%m-%d'),' 0:0:0')
                      AND tjrq <= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 2 DAY),'%Y-%m-%d'),' 23:59:59')
