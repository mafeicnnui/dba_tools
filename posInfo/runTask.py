...
def check_valid(p_cfg, p_task):    
    if p_task['task_type'] == 'timing':        
        if p_task['run_time'] <= get_time() \
          and (p_cfg.get('exe_time') is None \
              or get_day(p_cfg.get('exe_time')) <get_day(datetime.datetime.now())):
            p_cfg['exe_time'] = datetime.datetime.now()
            return True
        else:
            return False
