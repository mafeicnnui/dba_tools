...
def upd_status(p_dx_status,p_lt_status,p_market_id):
    db = get_db()
    cr = db.cursor()
    st = """insert into t_monitor_project_log(market_id,ip_dx_status,ip_lt_status,update_time)  values('{}','{}','{}',now())""".format(p_market_id,p_dx_status,p_lt_status)
    cr.execute(st)
    st = """update t_monitor_project  set ip_dx_status='{}', ip_lt_status='{}', update_time=now()where market_id='{}'""".format(p_dx_status,p_lt_status,p_market_id)
    print(st)
    cr.execute(st)
    db.commit()
...
def ping_server(po):
    ...
    if ip_dx !='':
       ip_dx_status = ping_window(ip_dx)
    else:
       ip_dx_status = ''

    print('Ping Server {}, please wait...'.format(ip_lt))
    if ip_lt !='':
       ip_lt_status = ping_window(ip_lt)
    else:
       ip_lt_status = ''
