...
def write_doc_digest_old(cfg,doc):
...
def write_doc_digest(cfg,doc):
    for item in get_tj_item(cfg, 'week_detail'):
        ...
        if item['item_code'] in ('body'):
            ...
            for r in res:
                table.rows[i].height   = Cm(0.7)
                table.cell(i, 0).width = Cm(3.0)
                table.cell(i, 1).width = Cm(3.5)
                table.cell(i, 4).width = Cm(2.8)
                j = 0
                for c in r:
                    table.cell(i, j).paragraphs[0].paragraph_format.alignment    = WD_TABLE_ALIGNMENT.CENTER
                    table.cell(i, j).paragraphs[0].paragraph_format.space_before = Pt(0.2)
                    ...
                    if i == 0:
                        ...
                        cell = table.cell(i, j)
                        p = cell.paragraphs[-1]
                        cell.paragraphs[-1].alignment = WD_PARAGRAPH_ALIGNMENT.CENTER  # 水平居中
                        ...
                        if r[c] != '':
                            if r[c].count('g')>0 or r[c].count('r')>0 or r[c].count('y')>0 :
                                for k in r[c].split(','):
                                    if k == 'g':
                                        run.add_picture('./images/green.png', width=Inches(0.25))
                                    if k == 'r':
                                        run.add_picture('./images/red.png', width=Inches(0.23))
                                    if k == 'y':
                                        run.add_picture('./images/yellow.png', width=Inches(0.23))
                            else:
                                run2 = table.cell(i, j).paragraphs[0].add_run(r[c])
                                run2.font.size = Pt(8)
                        else:
