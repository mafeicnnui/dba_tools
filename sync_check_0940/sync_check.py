...
def get_html_contents(config):
    ...
    v_html = '''<html>
                    <head>
                       <style type="text/css">
                           ...
                           .xwtable td{line-height:20px;text-align: right;padding:4px 10px 3px 10px;height: 18px;border: 1px solid #ccc;}
                       </style>
                    </head>
                    <body>
    ...
    v_sql  = '''SELECT 
                  ...
                  CASE WHEN t.market_id='218' THEN '北京朝阳合生汇'
                       ...
                       WHEN t.market_id='170' THEN '广州嘉和北'
                       ...
                       WHEN t.market_id='278' THEN '广州海珠合生新天地' 
                       WHEN t.market_id='287' THEN '西安南门合生汇'
                  END AS '项目名称',
                  t.tjrq             as '统计日期',
                  t.bi_flow          as '项目客流数',
                  t.bi_park          as '项目车流数', 
                  -- t.bi_sales_amount  as '项目销售额',
                  FORMAT(t.bi_sales_amount/100,2)  as '项目销售额',
                  t.bi_sales_count   as '项目销售笔数',
                  t.big_flow         as '大数据客流' ,
                  t.big_park         as '大数据车流',
                  -- t.big_sales_amount as '大数据销售额',
                  FORMAT(t.big_sales_amount/100,2) AS '大数据销售额',
                  t.big_sales_count  as '大数据销售笔数',        
                  CASE WHEN ABS((t.bi_flow-t.big_flow))/t.bi_flow <=0.05 THEN '√' 
                       WHEN ABS((t.bi_flow-t.big_flow))/t.bi_flow IS NULL THEN '√' 
                  ELSE  '×' END AS '客流对比',
                  CASE WHEN ABS((t.bi_park-t.big_park))/t.bi_park <=0.05 THEN '√'
                       WHEN ABS((t.bi_park-t.big_park))/t.bi_park IS NULL THEN '√'
                  ELSE  '×' END AS '车流对比',
                ...
                WHERE tjrq >= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                  AND tjrq <= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
             '''
    cr.execute(v_sql)
    rs    = cr.fetchall()
    desc  = cr.description
    ...
    if p_market_id == '218':
       v_stats = '''select IFNULL(SUM(insum),0) from hopsonone_flow_bj.dbo_summary_day
                         where market_id={0} and sitetype=300
                           and countdate>=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                           and countdate<=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                 '''.format(p_market_id)
       ...
       v_stats = '''select IFNULL(SUM(insum),0) from hopsonone_flow_sh_tjd.dbo_summary_day
                        where market_id={0} and sitetype=300
                          and countdate>=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                          and countdate<=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                '''.format(p_market_id)
    elif p_market_id in('164'):
       v_stats = '''select IFNULL(SUM(insum),0) from hopsonone_flow_bj_hs_square.dbo_summary_day
                        where market_id={0} and sitetype=300
                          and countdate>=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                          and countdate<=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                '''.format(p_market_id)
    elif p_market_id in('213'):
       v_stats = '''select IFNULL(SUM(insum),0) from hopsonone_flow_bj_wangjing.dbo_summary_day
                        where market_id={0} and sitetype=300
                          and countdate>=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                          and countdate<=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                '''.format(p_market_id)
       ...
       v_stats = '''select IFNULL(SUM(insum),0) from hopsonone_flow_zc.dbo_summary_day
                         where market_id={0} and sitetype=300
                           ...
                           and countdate<=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                 '''.format(p_market_id)
    elif p_market_id == '278':
       v_stats = '''SELECT ifnull(intersect_enter,0) FROM hopsonone_flow_gz_fangzhi3.building_kpis_daily
                     WHERE DATE=DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 2 DAY),'%Y-%m-%d')'''
    elif p_market_id == '287':
       v_stats = '''select IFNULL(SUM(insum),0) from hopsonone_flow_xa.dbo_summary_day
                         where market_id={0} and sitetype=300
                           and countdate>=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                           and countdate<=concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                 '''.format(p_market_id)
...
def get_bi_park_stats(config,p_market_id):
    ...
    if p_market_id=='218':
        v_stats = '''select COUNT(platenumpic) 
                      ...
                      where controlid IN (SELECT controlid FROM hopsonone_park_bj_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                        and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                        and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''
    elif p_market_id=='110':
        v_stats = '''select COUNT( platenumpic) 
                       from hopsonone_park_sh_wjc_tjd.dbo_park_caroutrun
                      where controlid IN (SELECT controlid FROM hopsonone_park_sh_wjc_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                        AND passdate>'2020-10-15'
                        and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                        and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''
    elif p_market_id=='108':
        ...
        #                and intime >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
        #                and intime <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
        #          '''.format(p_market_id)
       
        #v_stats = '''select COUNT( platenumpic) 
        #               from hopsonone_park_cd_tjd.dbo_park_caroutrun
        #              where controlid IN (SELECT controlid FROM hopsonone_park_cd_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
        #                AND passdate>'2020-10-15'
        #                and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
        #                and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
        #          '''
        v_stats = '''
                     select                         
                            t1.park_count + IF(t2.park_count IS NULL, 0, t2.park_count) 
                        FROM hopsonone_market_analysis.park_ready_caroutlog t1
                          LEFT JOIN hopsonone_market_analysis.park_ready_carlog t2 ON (t1.park_date = t2.park_date) 
                           WHERE  t1.park_date >= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                                AND  t1.park_date <= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''

    elif p_market_id=='132':
        v_stats = '''select COUNT( platenumpic) 
                       from hopsonone_park_gz_tjd.dbo_park_caroutrun
                      where controlid IN (SELECT controlid FROM hopsonone_park_gz_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                        and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                        and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''         
    elif p_market_id=='170':
        v_stats = '''select COUNT( platenumpic) 
                       from hopsonone_park_gz_north_tjd.dbo_park_caroutrun
                      where controlid IN (SELECT controlid FROM hopsonone_park_gz_north_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                        and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                        and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''         
    elif p_market_id in('164'):
        v_stats = '''select park_flow
                      ...
                      where market_id={0} 
                        and date(park_date) >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                        and date(park_date) <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                        limit 1
                  '''.format(p_market_id)
    elif p_market_id in ('213'):
        v_stats = '''select COUNT( platenumpic) 
                       from hopsonone_park_bj_wangjing_tjd.dbo_park_caroutrun
                      where controlid IN (SELECT controlid FROM hopsonone_park_bj_wangjing_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                        and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                        and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''
    elif p_market_id in('234'):
        v_stats = '''select COUNT( platenumpic) 
                     ...
                     where controlid IN (SELECT controlid FROM hopsonone_park_sh_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                       and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                       and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''

    elif p_market_id in('237'):
        v_stats = '''select COUNT( platenumpic) 
                      from hopsonone_park_gz_zc_tjd.dbo_park_caroutrun
                     where controlid IN (SELECT controlid FROM hopsonone_park_gz_zc_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                       and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                       and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''
    elif p_market_id in('278'):
        v_stats = '''select COUNT( platenumpic) 
                      from hopsonone_park_gz_fangzhi3_tjd.dbo_park_caroutrun
                     where controlid IN (SELECT controlid FROM hopsonone_park_gz_fangzhi3_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
    ...
    elif p_market_id in('287'):
        v_stats = '''select COUNT( platenumpic) 
                      from hopsonone_park_xa_tjd.dbo_park_caroutrun
                     where controlid IN (SELECT controlid FROM hopsonone_park_xa_tjd.dbo_Hardware_Control WHERE CarRunType = 1)
                       and passdate >= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                       and passdate <= concat(date_format(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
                  '''
...
def get_bi_sales_amount_stats(config,p_market_id):
    d_tjrq = (datetime.datetime.now() + datetime.timedelta(days=-1)).strftime('%Y-%m-%d')
    db_desc = config['db_bi']
    cr_desc = db_desc.cursor()
    v_stats = '''SELECT IFNULL(SUM(b.adjust_tax_sales_amount_month),0) 
                   FROM shop_side_operation_real_time.sales_report_day a,
                        shop_side_operation_real_time.sales_report_details b
...
def check_valid(config):
    ...
    sql='''SELECT COUNT(0)
           FROM (
             SELECT 
                market_id,
                CASE WHEN ABS((t.bi_sales_count-t.big_sales_count))/t.bi_sales_count <=0.05 THEN '√' 
	             WHEN ABS((t.bi_sales_count-t.big_sales_count))/t.bi_sales_count IS NULL THEN '√' 
                ELSE  '×' END AS 'sales_count',
                CASE WHEN ABS((t.bi_sales_amount-t.big_sales_amount))/t.bi_sales_amount <=0.05 THEN '√'
	             WHEN ABS((t.bi_sales_amount-t.big_sales_amount))/t.bi_sales_amount IS NULL THEN '√'
                ELSE  '×' END AS 'sales_amount'
            FROM  sync.t_sync_stats t
              WHERE tjrq >= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 0:0:0')
                AND tjrq <= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 DAY),'%Y-%m-%d'),' 23:59:59')
             ) X WHERE x.sales_count='×' OR x.sales_amount='×'
        '''
    cr.execute(sql)
    rs=cr.fetchone()
    print('rs=',rs,rs[0])
