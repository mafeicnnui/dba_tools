...
def write_key_index(p_rq_start,p_rq_end,p_week_rq):
    ...
    where a.market_id in (110,108,218,132)
    ...
    merchant_entity.market ma
    where m.market_id in (110,108,218,132)
    ...
    and b.market_id in (110,108,218,132)
    ...
    and p.market_id in (218,110,108,132)
    ...
    merchant_entity.market ma
    where p.status=2
    and p.opt_market in (108,110,218,132)
    ...
    merchant_entity.market ma
    where a.market_id in (108,110,218,132)
    and a.market_id=ma.id
    ...
    SELECT
      a.`商场名称`,
      b.name AS '购买渠道',
      SUM(CASE WHEN a.`购买渠道`=b.name THEN a.`购买量`  ELSE 0 END) AS '购买量'
    FROM 
      (select ma.market_name as '商场名称',
        ...
        and p.create_time<=concat(%(date_ed)s,' 23:59:59')
        and p.opt_market=ma.id
        group by p.opt_market,p.channel) a,
        (SELECT * FROM `shop_side_operation_real_time`.`sales_report_dmmx` WHERE dm='008') b
    GROUP BY 1,2
    """
    # 计时
...
if __name__ == '__main__':
    ...
    start_rq, end_rq = ('2024-09-01','2024-09-30')
