...
def write_pos_week(p_rq_start,p_rq_end,p_week_rq):
    ...
    合生通项目组.to_excel(项目文件夹 + '/' + '合生通项目组-POS月报' + 周报日期 + '.xlsx', index=False)

if __name__ == '__main__':
    # add by ma.fei
    output_dir = './out/POS月报/'
    os.system('rm -rf ./out/POS月报/*')
    ...
    # 文件打包
    path_original = 'out/POS月报'
    path_zip = './out/POS月报/POS月报-{}.zip'.format(week_rq)
    zip_name = 'POS月报-{}.zip'.format(week_rq)
