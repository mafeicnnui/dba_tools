...
def get_html_contents(config):
    ...
    v_sql = '''SELECT 
               ...
               WHERE create_date=(SELECT MAX(create_date) FROM sync.t_sync_flow_stats)
                 AND t.db IN('hopsonone_flow_bj','hopsonone_flow_sh','hopsonone_flow_cd','hopsonone_flow_gz','hopsonone_flow_zc','hopsonone_flow_bj_wangjing')
            '''
    cr.execute(v_sql)
    rs = cr.fetchall()
    desc = cr.description
    ...
    v_sql = '''SELECT 
                   ...
                   WHERE create_date=(SELECT MAX(create_date) FROM sync.t_sync_flow_stats)
                     AND t.db IN('hopsonone_flow_bj_real_time','hopsonone_flow_sh_real_time',
                                 'hopsonone_flow_cd_real_time','hopsonone_flow_gz_real_time','hopsonone_flow_zc_real_time','hopsonone_flow_bj_wangjing_real_time')
                '''
    cr.execute(v_sql)
    rs = cr.fetchall()
    desc = cr.description
...
def write_flow_stats(config):
    ...
    for p in config['market_id_flow'].split(","):
        ...
        v_stats_sql ='''insert into sync.t_sync_flow_stats
                        ...
                        WHERE t.market_id='{5}' AND t.countdate>DATE(DATE_ADD(NOW(),INTERVAL -2 DAY))
...
def write_flow_real_stats(config):
    ...
    for p in config['market_id_real_flow'].split(","):
        ...
        v_stats_sql ='''insert into sync.t_sync_flow_stats
                        ...
                        WHERE t.market_id='{5}' and countdate>DATE(DATE_ADD(NOW(),INTERVAL -1 hour))
                     '''.format(v_market_db,v_market_db,v_status,v_create,v_market_db,v_market_id)
        cr.execute(v_stats_sql)
