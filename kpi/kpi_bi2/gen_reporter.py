...
def get_hz_log(dbd):
   ...
   st="""select a.bbrq,a.month,
        ...
        ORDER BY b.sxh,a.item_code+0""".format(get_month())
   cr.execute(st)
   rs=cr.fetchall()
   return rs
...
def check_item_exists(db,po):
    ...
    st = """select  count(0)  as rec from `kpi_po_mx` where market_id='{}' and item_code= '{}' and month='{}'
         """.format(po['market_id'],po['item_code'],get_month())
    cr.execute(st)
    rs = cr.fetchone()
    cr.close()
    return rs['rec']
...
def check_hz_exists(db):
    cr = db.cursor()
    st = """select  count(0)  as rec from `kpi_po_hz` where `month`='{}'""".format(get_month())
    cr.execute(st)
    rs = cr.fetchone()
    cr.close()
    return rs['rec']
...
def write_log(db,bb):
    ...
    for po in bb:
        if check_item_exists(db,po)>0:
            msg = 'update market:{} item:{} log , value={},sum={} ....'.format(po['market_id'],po['item_name'],po['value'],po['value_sum'])
            print(msg)
            st = """update kpi_po_mx 
                       set  item_value='{}',
                            item_value_sum='{}',
            ...
            st = """insert into kpi_po_mx(market_id,market_name,bbrq,month,item_code,item_name,item_value,item_value_sum,create_time) 
                      values('{}','{}','{}','{}','{}','{}','{}','{}',now())
                   """.format(po['market_id'],po['market_name'],get_bbrq(),get_month(),po['item_code'],po['item_name'],po['value'],po['value_sum'])
        cr.execute(st)
        wrtie_task_detail_log(db, msg)
        db.commit()
...
def update_log(db):
    ...
    st = """insert into kpi_po_mx(bbrq,month,market_id,market_name,item_code,item_name,item_value,item_value_sum,create_time,update_time)
            select '{}',
                    ...
                    NOW() AS update_time  
            from kpi_po_mx 
            where item_code IN('2.1','2.2') and month='{}'
            group by month,market_id,market_name""".format(get_bbrq(),get_month())
    cr.execute(st)
    wrtie_task_detail_log(db, '生成项目区域：总GMV指标（万）...')
    db.commit()
    ...
    INSERT INTO kpi_po_mx(bbrq,MONTH,market_id,market_name,item_code,item_name,item_value,item_value_sum,create_time,update_time)
            SELECT 
                ...
                '线上GMV/线下销售额' AS item_name,
                 CONCAT(ROUND(ROUND((SELECT item_value FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='2')/
	              (SELECT item_value FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='9'),4)*100,2),'%') AS item_value,
	             CONCAT(ROUND(ROUND((SELECT item_value_sum FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='2')/
	              (SELECT item_value_sum FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='9'),4)*100,2),'%') AS item_value_sum,
                NOW() AS create_time,
                NOW() AS update_time  
            ...
            FROM kpi_po_mx a WHERE a.month='{}' AND a.market_id=40000 AND a.item_code='9'
                """.format(get_month(),get_month(),get_month())
    cr.execute(st)
    wrtie_task_detail_log(db, '生成项目区域：线上GMV/线下销售额')
    db.commit()
    ...
    st = """INSERT INTO kpi_po_mx(bbrq,MONTH,market_id,market_name,item_code,item_name,item_value,item_value_sum,create_time,update_time)
                     SELECT '{}',
                           ...
                           NOW() AS update_time  
                     FROM kpi_po_mx 
                     WHERE item_code IN('12.1','12.2') and month='{}'
                     GROUP BY MONTH,market_id,market_name""".format(get_bbrq(),get_month())
    cr.execute(st)
    wrtie_task_detail_log(db, '生成总部：总GMV指标（万）...')
    db.commit()
    ...
    st = """
      INSERT INTO kpi_po_mx(bbrq,MONTH,market_id,market_name,item_code,item_name,item_value,item_value_sum,create_time,update_time)
          SELECT 
              ...
              '15' AS item_code,
              '线上GMV/线下销售额' AS item_name,
              CONCAT(ROUND(ROUND((SELECT item_value FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='12')/
                    (SELECT item_value FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='13'),4)*100,2),'%') AS item_value,
              CONCAT(ROUND(ROUND((SELECT item_value_sum FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='12')/
                    (SELECT item_value_sum FROM kpi_po_mx b WHERE a.month=b.month AND a.market_id=b.market_id AND b.item_code='13'),4)*100,2),'%') AS item_value_sum,
              NOW() AS create_time,
                  NOW() AS update_time  
...
def update_hz(db):
    ...
    if check_hz_exists(db) == 0:
        ...
        wrtie_task_detail_log(db, 'insert kpi_po_hz log...')
        st = """INSERT INTO `kpi_po_hz`(bbrq,MONTH,market_id,market_name,item_code,item_name,actual_completion,completion_sum_finish,create_time)
                  ...
                  FROM kpi_po_mx
                  WHERE MONTH='{}'""".format(get_month())

    else:
        print('update kpi_po_hz log....')
        wrtie_task_detail_log(db, 'update kpi_po_hz log...')
                      ...
                      SET actual_completion=(SELECT item_value FROM kpi_po_mx b
                          ...
                          bbrq=(SELECT bbrq FROM kpi_po_mx b
                                              WHERE b.month=a.month 
                                                ...
                                                AND b.item_code=a.item_code)
                    WHERE MONTH='{}' 
                    and  a.item_code not in(SELECT b.code FROM `kpi_item` b WHERE b.if_stat='N') """.format(get_month())
    cr.execute(st)
    db.commit()

def update_task_log(db,p_total,p_counter):
    cr = db.cursor()
    if p_counter==0:
       cr.execute("""delete from  kpi_log_detail where month='{}'""".format(get_month()))
    cr.execute("select count(0) as rec from kpi_log where month='{}'".format(get_month()))
    rs=cr.fetchone()
    if rs['rec']==0:
       cr.execute("""
          insert into kpi_log(month,bbrqq,bbrqz,create_date,duration,percent) values('{}','{}','{}',now(),'{}','{}')
       """.format(get_month(),get_first_day(),get_last_day(),0,'0%'))
       cr.execute("""delete from  kpi_log_detail where month='{}'""".format(get_month()))
    else:
       cr.execute("""
          update kpi_log  set  duration=TIMESTAMPDIFF(SECOND,create_date,now()),percent='{}' where month='{}'
       """.format(str(round(p_counter / p_total * 100, 2))+'%',get_month()))


def wrtie_task_detail_log(db,p_msg):
    cr = db.cursor()
    cr.execute("""insert into kpi_log_detail(month,message,create_date) values('{}','{}',now())
    """.format(get_month(),p_msg))
...
if __name__ == '__main__':
    ...
    for market in get_markets(dbd,'N'):
        ...
        # 写日志
        update_task_log(dbd, total,counter)
        write_log(dbd,bb)
