        ...
        v_stats = '''
			...
			FROM hopsonone_market_analysis.park_ready_caroutlog t1
			  LEFT JOIN hopsonone_market_analysis.park_ready_carlog t2 ON (t1.park_date = t2.park_date) 
			   WHERE  t1.park_date >= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 2 DAY),'%Y-%m-%d'),' 0:0:0')
                        	AND  t1.park_date <= CONCAT(DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 2 DAY),'%Y-%m-%d'),' 23:59:59')
                  ''' 
...
def main():
    ...
    if not check_hadoop_status(config):
      ...
      #send mail
      send_mail465(config['send_user'], config['send_pass'],config['acpt_user'], config['mail_title'],get_html_contents(config))
    else:
